# 2026-08-27 — reframe claim: an explicit poll budget and a decoupled lease floor

**MONEY. adgen half of a two-repo change. `liquidretail_backend` PR #355 must
merge BEFORE this one — see "Merge order" below, and note this PR's suite is
EXPECTED RED on exactly one check until it does.**

## What this repo contributes, and what it already had

The shutdown claim eviction (`_activeReframeClaims` +
`releaseAllActiveReframeClaims`, swept from `renderer.js`'s `shutdown()`)
**originated here and is unchanged by this PR.** Backend had neither, which is
why every backend process death — twelve or more web deploys on 2026-08-27 alone
— left reframe billing claims standing in Mongo with no live holder (two such
orphans, both with dead holders, found in production). That port is PR #355's
work; this PR only adds a regression fence around the mechanism so it cannot
quietly disappear from the repo it came from.

What IS new here is the half that was always adgen's: this repo raised
`ATLAS_TIMEOUT_MS` to 900000 in #82 and, because the lease floor was derived from
it, moved its own stale-claim line to 25 min while backend stayed at 20 — over a
claim on a **shared** `Media` document.
...
was **3.9x** the observed max here (2.6x in backend).
...
1. **Cross-repo drift.** The claim is a field on the **shared** `Media` document
   that backend also steals from using its own copy of the formula. #82 raised
   **this** repo's `ATLAS_TIMEOUT_MS` to 900000 and left backend at 600000, so the
   two sides silently disagreed about when a holder is dead — **25 min here, 20
   min there.**
...
2. **The lease floor is now an independent constant**,
   ...
   other.
   ⚠️ **This repo's floor value DROPS 25 min → 20 min, and that is not a loss of
   safety.** The extra 5 minutes was illusory: the claim is on a shared document
   and backend steals at ITS floor (20 min) regardless of what this repo believes,
   so the effective system-wide steal threshold has always been 20 min. This makes
   this repo's belief match the system's actual behaviour. The real margin comes
   from the poll budget, not from the floor.
...
4. **A regression fence** (group D) around the shutdown claim eviction this repo
   already had, so it cannot silently disappear from its repo of origin.
...
- **Baseline on pristine `origin/master` (`9f41215`), this worktree, no
  `NODE_PATH` and no `node_modules` per this repo's CLAUDE.md:** **76/78 passed**,
  1 expected-fail (`verifyRunFinalizesOnSettle_KNOWN_OPEN.js`), 1 FAILED
  (`verifyVendorDrift.js` — pre-existing, four adgen entries plus two synced-drift
  and one dead module).
- **After:** see "Expected red" below. `verifyVendorDrift.js` is reconciled by this
  PR for the file it touches, and `verifySharedInvariants.js` is red BY DESIGN
  until backend #355 merges.
