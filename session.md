...
## 2026-08-19 — Second adversarial pass on the grounded-Gemini ledger fix
...
($0.035), so 92-94% of each call's cost was invisible — the same defect `match()` had.
...
- **Was still open — NOW CLOSED by `fix/gemini-grounded-cost-ledger` (see the section at the
  top of this file):** `categoryReviewsService` and `productDetailsService.fetchReviewSummary`
  both POSTed the raw direct endpoint with no `trackLlmCall`. Both genuinely grounded, so the
  fix was "wrap in a ledgered transport," not "move to Atlas" — exactly as predicted here.
