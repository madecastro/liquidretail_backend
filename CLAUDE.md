...
- **Ingest freshness (`feat/ingest-freshness`).** Title/description change (normalised) clears `shortBenefitsDerivedAt` and re-enqueues derivation; price/image/URL-only does not. Scheduled re-sync now covers Shopify-direct / generic / Apify as well as IG (`CATALOG_SCHEDULED_RESYNC_ENABLED`, parser `=== 'true'`). `imageUrl` upsert omits null (heal null→url, never clobber url→null). Pinned by `verifyProductBenefits.js` F-group + `verifyScheduledCatalogResync.js`.
