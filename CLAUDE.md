...
- **Ingest freshness (`feat/ingest-freshness`).** Title/description change (normalised) clears `shortBenefitsDerivedAt` and re-enqueues derivation; price/image/URL-only does not. Scheduled re-sync covers Shopify-direct / generic / Apify as well as IG (`CATALOG_SCHEDULED_RESYNC_ENABLED`, parser `=== 'true'`): nightly 2am Pacific window, all brands, persist-uncapped on that path only. `imageUrl` upsert omits null (heal null→url, never clobber url→null). Pinned by `verifyProductBenefits.js` F-group + `verifyScheduledCatalogResync.js`.
