...
'use strict';
/**
 * PORTED from liquidretail_backend/scripts/verifyFaceSafeCrop.js into
 * liquidretail_adgen. Path adjustment only (../services/faceSafeCrop ->
 * ../src/services/faceSafeCrop); every assertion below is unchanged from the
 * backend original, plus L3/L4 pinning FACE_QUORUM_RETRY_FRAMES.
 *
 * verifyFaceSafeCrop — offline correctness suite for src/services/faceSafeCrop.js.
 *
...
const fsc = require('../src/services/faceSafeCrop');
const {
  ...
  FACE_MAX_SUBJECT_AREA_FRAC, FACE_MIN_FRAMES, FACE_QUORUM_RETRY_FRAMES,
} = fsc;
...
check('L3 FACE_QUORUM_RETRY_FRAMES is 2 (bounded one-shot, not an env knob)', () => {
  assert.strictEqual(FACE_QUORUM_RETRY_FRAMES, 2);
  assert.ok(FACE_QUORUM_RETRY_FRAMES >= FACE_MIN_FRAMES,
    'retry batch must be able to finish a 1-hit quorum');
});
check('L4 FACE_QUORUM_RETRY_FRAMES is a plain code constant, not env-tunable', () => {
  const fs = require('fs');
  const path = require('path');
  const src = fs.readFileSync(path.join(__dirname, '..', 'src/services/faceSafeCrop.js'), 'utf8');
  assert.ok(/const FACE_QUORUM_RETRY_FRAMES = 2;/.test(src),
    'FACE_QUORUM_RETRY_FRAMES is not the expected plain constant');
  assert.ok(!/process\.env\.FACE_QUORUM_RETRY_FRAMES/.test(src),
    'FACE_QUORUM_RETRY_FRAMES must not be an env knob');
});
