...
check('P1 no narrow brand-titling projection survives (the defect class is moot, not un-fixed)', () => {
  // HISTORY. The defect this used to pin SHIPPED and reached the owner:
  // routes/ads.js (the wizard/generation path) and adRegenerateService
  // .loadBrand both .select()'d an explicit field list feeding
  // buildMetaForAd, and neither listed brandReviews — brandPair went null,
  // resolveAtomicRatingPair returned source=none, and every generated ad
  // rendered with NO stars and NO review count (including Vuori, 4.58 /
  // 15,545, which clears the >4.5 gate outright).
  //
  // CORRECTED 2026-08-28 (backend titling removal, owner directive: "remove
  // and disable the backend titling function"). Both narrow projections are
  // DELETED along with the in-process titling calls they fed — routes/
  // ads.js's renderOneInner brandDoc lookup and adRegenerateService's
  // loadBrand() no longer exist at all, so there is nothing left to check
  // field-by-field. This check now asserts that absence directly rather
  // than silently going green on a pattern match against zero occurrences
  // (which would prove nothing). The remaining live path that feeds a real
  // Brand doc to buildMetaForAd is routes/brand.js's manual retitle
  // endpoints, which load the FULL Mongoose document (no .select() at
  // all) — immune to this defect class by construction, which is why it
  // was never a target of this check.
  const targets = ['routes/ads.js', 'services/adRegenerateService.js'];
  const re = /\.select\('name styleScript[^']*'\)/g;
  for (const rel of targets) {
    const src = fs.readFileSync(path.join(__dirname, '..', rel), 'utf8');
    const matches = src.match(re) || [];
    assert.strictEqual(matches.length, 0,
      `${rel}: a narrow brand-titling projection reappeared (${matches.length} found) — ` +
      `if backend titling is ever reintroduced, it MUST include brandReviews or the proof ` +
      `beat silently loses stars AND review count again`);
  }
});
