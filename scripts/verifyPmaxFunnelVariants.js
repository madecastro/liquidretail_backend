 ...
 *   4. (retired) Add a veoGenerateForAd(...) call inside
 *      renderDeriveOnlyVideoAd / remove the early return after the
 *      derive gate. That function and the in-process render loop are
 *      gone. MONEY invariant "a funnel-variant ad must never reach a
 *      billable Omni submit" is still enforced by resolveDeriveFromMaster
 *      at mint/preflight (D-group / RP1 / F-group regenerate) + adgen's
...
// E0/E0b/E1/E2/E3 extracted renderDeriveOnlyVideoAd and pinned ZERO
// billable submits + the derive gate before Omni. E4–E7 extracted /
// live-executed findSiblingMasterAd (true-masters-only, no calendar-day
// bound, PMax 10s floor). Both functions were deleted with the in-process
// render loop; the sibling lookup moved to adgen's renderer. MONEY
// invariant "a funnel-variant ad must never reach a billable Omni submit"
// is still enforced by resolveDeriveFromMaster at mint/preflight — still
// pinned in this file's D-group, RP1, and F-group regenerate preflight.
...
check('E-abs [ABSENCE] routes/ads.js no longer defines renderDeriveOnlyVideoAd',
  ...
  'the in-process derive renderer came back — restore the E0–E3 money pins');
