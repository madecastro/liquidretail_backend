...
function restoreEnv() {
  ...
  try {
    delete require.cache[require.resolve('../src/services/adCopyGuards')];
...
function loadIntents(flag) {
  ...
  const guardKey = require.resolve('../src/services/adCopyGuards');
...
function loadGuards(flag) {
  const key = require.resolve('../src/services/adCopyGuards');
  delete require.cache[key];
  if (flag === undefined) delete process.env.STATIC_RATING_FURNITURE;
  else process.env.STATIC_RATING_FURNITURE = flag;
  return require('../src/services/adCopyGuards');
}

function loadDirector() {
  const key = require.resolve('../src/services/aiCreativeDirectorService');
  delete require.cache[key];
  delete require.cache[require.resolve('../src/services/adCopyGuards')];
  return require('../src/services/aiCreativeDirectorService');
}
...
try {
  ...
  console.log('E. fallback, intent data, scope label');
  {
    ...
    const dirKey = require.resolve('../src/services/directImageRenderService');
    delete require.cache[dirKey];
    const { buildIntentData } = require('../src/services/directImageRenderService');
