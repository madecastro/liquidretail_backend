...
// OF SPENDING: this module imports only atlasPoll + geminiPoll + manifest —
// it must never reference either provider's billable submit path or
// assemble a request body (the verify harness scans this file's text for
// those identifiers, which is why this comment names neither). A cell that
// failed BEFORE a receipt existed stays failed; re-running it is a new
// experiment the operator triggers deliberately with `rpd run`.
...
const { settleCell: settleGeminiCell, fetchSettledCost: fetchSettledGeminiCost } = require('./geminiPoll');
// Deliberately a LITERAL, not read off the real service's PROVIDER export —
// requiring that module here would put its submit-capable exports back
// within this file's own require graph, which is exactly the structural
// guarantee this file exists to avoid (same reasoning as not naming the
// submit functions above). The real service's exported value is 'gemini'.
...
async function recoverStaticCells(runDir, manifest, cells) {
  ...
  const { peekImagePrediction } = require('../../../src/services/atlasImageService');
  let recovered = 0;
...
async function resumeRun(runDir) {
  ...
  for (const cell of unpriced) {
    if (cell.provider === GEMINI_PROVIDER) {
      const { costUsd, costSource } = await fetchSettledGeminiCost(cell.predictionId, cell.resolution);
      if (costUsd != null) {
        cell.costUsd = costUsd;
        cell.costSource = costSource;
        writeManifest(runDir, manifest);
        console.log(`  💲 ${cell.id}: settled cost $${costUsd.toFixed(2)}`);
      }
      continue;
    }
