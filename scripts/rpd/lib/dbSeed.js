...
async function resolveSeedFromDb(productId) {
  ...
  try {
    ...
    // Full titling-ready field set — the same ones titling.js's fixtureBrand()
    // fakes (logoUrl/colors/fontFamily/tagline/titleStylePreset/styleTheme).
    // Fetched here so a real product's titled video can carry its REAL look
    // instead of the fixture's, without titling.js touching Mongo itself.
    const brand = product.brandId
      ? await Brand.findById(product.brandId)
        .select('name websiteBackground primaryColor secondaryColor accentColor ' +
    ...
    return {
      ...
      // Real Brand doc, titling-ready shape. NEVER carries proof-class
      // fields (quote/rating/reviewCount) — Brand doesn't have them; those
      // stay absent-by-default in titling.js regardless of this object.
      brand: brand ? {
