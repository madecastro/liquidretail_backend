...
function recordColumn(name, urlsByAdId, extra = {}) {
...
(async () => {
  ...
  const adIds = pool.ads.map((a) => a.adId);
  ...
  if (PHASES.includes('baseline')) {
    if (DRY_RUN) log('(dry-run) would record baseline URLs');
      ...
      if (existing?.columns?.includes('baseline')) {
        ...
        recordColumn('baseline', fromPool, { source: 'pool selection', note: 'pre-experiment renders' });
      }
    }
  }
  ...
  if (PHASES.includes('armA')) {
    ...
    if (!DRY_RUN) recordColumn('armA', await currentUrls(adIds), { engine: 'canonical + monochrome ink + font ladder' });
  }
