 ...
 * no trackLlmCall, so they billed Google whenever Google's own daily grounding
 * allowance was exceeded — Google's published per-request grounding surcharge,
 * which would dwarf the token cost — and wrote NOTHING to CostLog either way.
 * A $0 path is indistinguishable from a free one; that is how they stayed
 * invisible, and it is the same defect `match()` had.
 *
 * ⚠️ WHAT `GROUNDING_PER_CALL` ACTUALLY IS TODAY, so a future reader does not
 * mistake a near-zero `costUsd` on a `groundedRequests:1` row for a bug: it is
 * read LIVE from `costTracker.GROUNDED_SEARCH_COST_PER_REQUEST_USD`, whose
 * default is currently $0 — a separate, deliberate, extensively-documented
 * decision (see that constant in services/costTracker.js) made because measured
 * grounded volume sits comfortably inside Google's 1,500/day free allowance.
 * The FIX this file guards — every grounded call correctly ledgered, with
 * `groundedRequests` declared so the free-allowance counter and any future
 * repricing see it — is unaffected by which number that constant currently
 * holds; that is deliberate too, see F0/F4 below.
 *
...
// DERIVED FROM costTracker, NOT A HARDCODED LITERAL (hardened 2026-08-19, third
// pass — found by a REBASE, not a reviewer). This used to be a bare `0.035`
// ("$35 / 1,000 grounded prompts"). While this PR was in flight, a separate
// session landed a deliberate, extensively-documented change to
// `GROUNDED_SEARCH_COST_PER_REQUEST_USD`'s default: Google's 1,500/day free
// grounded-prompt allowance covers measured volume (13-19/day) many times over,
// so the ledger now prices grounding at $0 by default and alerts a human
// instead of auto-repricing if volume approaches the allowance (see the long
// comment above that constant in services/costTracker.js — it explicitly says
// "if you are here to finish this by auto-repricing, that is the regression").
// A hardcoded `0.035` here would have kept asserting the OLD default forever,
// silently drifting from reality the moment F0's cross-check below wasn't
// enough to save it. Reading it live means this harness tracks whatever the
// real default is, whatever it becomes next.
const GROUNDING_PER_CALL = costTracker.GROUNDED_SEARCH_COST_PER_REQUEST_USD;
...
await checkAsync('F0 this file\'s rate constants still match costTracker\'s live table', async () => {
  ...
  // No comparison against GROUNDING_PER_CALL here: it IS
  // costTracker.GROUNDED_SEARCH_COST_PER_REQUEST_USD (read live, not a second
  // hardcoded copy — see that constant's own declaration above), so comparing
  // them would only ever assert x === x.
});
...
await checkAsync('F4 the grounding surcharge is ADDITIVE and correctly GATED on grounded:true/false', async () => {
  // REWRITTEN 2026-08-19, third pass, by a REBASE not a reviewer: this used to
  // assert "grounding is the MAJORITY of the row" — true only under the OLD
  // GROUNDED_SEARCH_COST_PER_REQUEST_USD default of $0.035/call. While this PR
  // was in flight, a separate session deliberately changed that default to $0
  // (Google's 1,500/day free grounded-prompt allowance comfortably covers
  // measured volume — see the long comment above that constant in
  // services/costTracker.js). A "dominates" claim is regime-specific and would
  // have started FAILING on correct code the moment that landed on `main`, the
  // exact "harness rots the same way every verified-once claim does" trap.
  // What survives any regime: the surcharge is ADDED to token cost (never
  // multiplies or replaces it), and it is applied ONLY where `groundedRequests`
  // is actually 1 — checked here structurally, against GROUNDING_PER_CALL's
  // LIVE value (imported from costTracker above), not a hardcoded number.
  rows.length = 0;
  stubAxios(twoTransportStub());
  await categorySvc.fetchCategoryReviews({ brandName: 'Pelagic Gear', breadcrumb: 'Men > Shirts' });
  restoreAxios();
  ...
  for (const r of grounded) {
    const tokenOnly = (r.inputTokens / 1e6) * FLASH_INPUT_PER_1M + (r.outputTokens / 1e6) * FLASH_OUTPUT_PER_1M;
    assert.strictEqual(r.costUsd, Number((tokenOnly + GROUNDING_PER_CALL).toFixed(6)),
      `${r.stage}: costUsd must be EXACTLY token cost plus the surcharge, whatever the surcharge currently is`);
  }
  const u = ungrounded[0];
  const uTokenOnly = (u.inputTokens / 1e6) * ATLAS_FLASH_INPUT_PER_1M + (u.outputTokens / 1e6) * ATLAS_FLASH_OUTPUT_PER_1M;
  assert.strictEqual(u.costUsd, Number(uTokenOnly.toFixed(6)),
    'the ungrounded pass must carry NO surcharge component, at any GROUNDING_PER_CALL value');
});
