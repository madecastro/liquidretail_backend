...
(async () => {
  ...
  const geminiPollSrc = read(path.join(RPD, 'lib', 'geminiPoll.js'));
  ...
  check('B2b geminiPoll.js never submits (no axios.post, no submitGeneration reference)', () => {
    // Gemini's billable primitive is also named submitGeneration (real
    // export, services/geminiVideoService.js) — same regex, same guarantee
    // as B2 for the Atlas side.
    assert(!/axios\.post|submitGeneration|generateForAd/.test(codeOnly(geminiPollSrc)));
  });
  check('B3 resume.js pulls in nothing billable, and only the free image read', () => {
    ...
    const allowed = new Set(['fs', 'path', './manifest', './atlasPoll', './geminiPoll', '../../../src/services/atlasImageService']);
  ...
  const { buildVeoPrompt, enforceRawByteCap, OMNI_DIRECTIVES } =
    require(path.join(__dirname, '..', 'src', 'services', 'veoPromptBuilder'));
  const { capsFor, BUILT_IN_DEFAULT_MODEL } =
    require(path.join(__dirname, '..', 'src', 'services', 'atlasVideoService'));
  ...
  check('D4 directives lever patches the singleton but CORE ignores it (2026-09-03 architecture)', () => {
    // buildVeoPrompt no longer reads OMNI_DIRECTIVES/GROK_DIRECTIVES/
    // HOOK_FIRST_DIRECTIVES into the assembled prompt at all — it pushes the
    // single frozen corePromptText(durationSec) unconditionally (see that
    // function's header comment: "CORE IS THE PROMPT", owner-directed
    // 2026-09-03). The `directives` lever still patches the object (and D5
    // still proves it restores byte-identically) but the patch is now
    // provably inert on output — asserting that INERTNESS is the honest
    // test, not asserting the old text shows up.
    const objective = 'Objective: Verification objective sentence, entirely distinct.';
    const baseline = buildForCell({ spec, model, caps, variant: { id: 'baseline' } }).prompt;
    const { prompt } = buildForCell({ spec, model, caps, variant: { id: 'd', directives: { objective } } });
    assert.strictEqual(prompt, baseline, 'CORE must ignore a patched OMNI_DIRECTIVES.objective');
  ...
  check('D7 patch lever enforces exactly-once find', () => {
    // The find target is real CORE text (post-2026-09-03 rewrite) — the old
    // "Smooth crossfades only" phrase was part of OMNI_DIRECTIVES.transitions,
    // which CORE does not emit at all (see D4).
    const { prompt } = buildForCell({
      spec, model, caps,
      variant: { id: 'p', patch: [{ find: 'AUDIO: natural ambience only', replace: 'Hard cuts only' }] }
    });
  ...
  check('E1 atlasVideoService exports submitGeneration for the harness', () => {
    const svc = require(path.join(__dirname, '..', 'src', 'services', 'atlasVideoService'));
  ...
  check('E3 duration snaps to the model enum and both values are recorded', () => {
    ...
    // CORE states duration once, up front ("{N}-second premium Meta product
    // commercial…") — there is no "Output:" line any more (that belonged to
    // the pre-2026-09-03 Timeline/Scene/Output directive format). It must
    // still be built from the EFFECTIVE (snapped) duration, not the request.
    assert(cells[0].prompt.includes(`${cells[0].durationSec}-second`));
  });
  ...
  check('E7 a lifestyle (ugc) build also gets CORE, and a LIFESTYLE_DIRECTIVES patch stays inert', () => {
    // Pre-2026-09-03: VIDEO_LIFESTYLE_PROMPT on + variantKind 'ugc' built
    // from LIFESTYLE_DIRECTIVES, a directive set DISTINCT from the Omni one —
    // patching the wrong singleton there was a silent no-op reported as an
    // experiment arm, which is what this check originally guarded against.
    // Since CORE IS THE PROMPT unconditionally (buildVeoPrompt pushes
    // corePromptText(durationSec) regardless of the computed `lifestyle`
    // flag — see that push site's header comment), a lifestyle build is now
    // BYTE-IDENTICAL to a packshot build at the same duration, and patching
    // LIFESTYLE_DIRECTIVES is inert for the same reason D4 documents for the
    // Omni set. The singleton-restore guarantee still matters and is still
    // asserted.
    const prev = process.env.VIDEO_LIFESTYLE_PROMPT;
    process.env.VIDEO_LIFESTYLE_PROMPT = 'true';
    try {
      const { LIFESTYLE_DIRECTIVES } = require(path.join(__dirname, '..', 'src', 'services', 'veoPromptBuilder'));
      const before = JSON.stringify(LIFESTYLE_DIRECTIVES);
      const objective = 'Objective: Lifestyle patch sentence, entirely distinct.';
      const packshotBaseline = buildForCell({ spec, model, caps, variant: { id: 'baseline' } }).prompt;
      const { prompt, promptMeta } = buildForCell({
        spec, model, caps,
        variant: { id: 'ugc-d', variantKind: 'ugc', directives: { objective } }
      });
      assert(!prompt.includes(objective), 'CORE must ignore a patched LIFESTYLE_DIRECTIVES.objective');
      assert.strictEqual(prompt, packshotBaseline, 'a lifestyle build is byte-identical to packshot under CORE');
      assert.ok(promptMeta.lever === 'directives');
  ...
  check('E9 a real DB-sourced brand wins titling over the fixture, and proof-class fields stay absent even then', () => {
    const { resolveTitleBrand, fixtureBrand, fixtureMeta } = require(path.join(RPD, 'lib', 'titling'));
    // No real brand supplied (pure prompt-testing spec, no seed.productId) —
    // must fall back to the fixture exactly as before this change.
    const fixtureOnly = resolveTitleBrand({ brandName: 'X' }, 'canonical');
    assert.deepStrictEqual(fixtureOnly, fixtureBrand('canonical', 'X'));

    // A real brand object (the shape dbSeed.js's resolveSeedFromDb returns)
    // must be used AS-IS — never re-wrapped in or merged with the fixture.
    const realBrand = {
      _id: 'real-brand-id', name: 'Real Brand', websiteUrl: 'https://real.example.com',
      tagline: 'Real tagline', logoUrl: 'https://real.example.com/logo.png',
      primaryColor: '#111111', secondaryColor: '#222222', accentColor: '#333333',
      fontFamily: 'Real Font', titleStylePreset: 'canonical', styleTheme: null
    };
    const resolved = resolveTitleBrand({ brand: realBrand, brandName: 'ignored' }, 'canonical');
    assert.strictEqual(resolved, realBrand, 'a supplied real brand object must be used as-is, not copied/mutated');
    assert.notStrictEqual(resolved.primaryColor, fixtureBrand('canonical', 'X').primaryColor,
      'a real brand must not silently inherit the fixture Pelagic palette');

    // The money/trust guarantee E8 already pins for the fixture must ALSO hold
    // when a real brand is in play — a real product/brand must never cause a
    // quote, rating, or review count to be fabricated into the chrome.
    const realMeta = fixtureMeta({}, realBrand);
    for (const k of ['quote', 'quoteSnippet', 'reviewer', 'rating', 'reviewCount', 'reviewsText', 'badgeText', 'deliveryLine']) {
      assert.strictEqual(realMeta[k], null, `${k} must stay null for a real brand too, unless the operator supplies it`);
    }
    // A real product's title IS safe to default as the headline — it is a
    // fact about the product, not an invented claim — mirroring how
    // runner.js wires resolveSeedFromDb's productTitle into spec.titling.copy.
    const realMetaWithHeadline = fixtureMeta({ headline: 'Real Product Title' }, realBrand);
    assert.strictEqual(realMetaWithHeadline.headline, 'Real Product Title');
    assert.strictEqual(realMetaWithHeadline.brandName, 'Real Brand');
    assert.strictEqual(realMetaWithHeadline.brandLogoUrl, 'https://real.example.com/logo.png');
  });
  check('E10 runner.js wires the real brand/headline from a DB seed only when the operator has not already set one', () => {
    // Structural, not behavioral — resolveSeedFromDb needs a live Mongo, same
    // tier as this repo's other DB-dependent wiring checks. What matters here
    // is the OVERRIDE PRECEDENCE: an operator-supplied spec.titling.brand or
    // spec.titling.copy.headline must never be clobbered by the DB seed.
    const src = fs.readFileSync(path.join(RPD, 'lib', 'runner.js'), 'utf8');
    assert(/if \(spec\.titling && !spec\.titling\.brand && resolved\.brand\)/.test(src),
      'must guard on !spec.titling.brand before wiring the real brand — an operator override must win');
    assert(/if \(!spec\.titling\.copy\.headline\) spec\.titling\.copy\.headline = resolved\.productTitle/.test(src),
      'must guard on !spec.titling.copy.headline before defaulting it from the real product title');
    // Must never wire proof-class fields from the DB seed — dbSeed.js's
    // return shape carries no quote/rating/reviewCount, and this line must
    // not start reading any onto spec.titling.copy from resolved.* either.
    for (const field of ['quote', 'rating', 'reviewCount', 'reviewsText']) {
      assert(!new RegExp(`resolved\\.${field}\\b`).test(src),
        `runner.js must never read resolved.${field} — dbSeed.js has no such field and none should be invented here`);
    }
  });
  ...
  const intentsMod = require(path.join(__dirname, '..', 'src', 'services', 'staticAdIntents'));
  const staticSpec = {
    name: 'verify-static',
  ...
  check('K1 the element catalog reads LIVE constants and covers every lever', () => {
    ...
    const veo = require(path.join(__dirname, '..', 'src', 'services', 'veoPromptBuilder'));
