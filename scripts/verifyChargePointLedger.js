...
// cannot classify — a bare Cloudflare 502 mid-poll is exactly that. The
// deleted renderCreative used to collapse that null to `false`; the remaining
// backend writer (settleChargeState) must never do that. Understating the
// ledger is the one direction it can never be corrected in.
...
// E2/E3 REMOVED 2026-09-07: the tri-state `chargeState: err.charged === true
// ? 'charged' : 'unknown'` / `charged: err.charged === true` write lived in
// deleted renderCreative. renderService now only exports composeVideoOutput
// and no longer records render failures. The tri-state write now lives in
// adgen's renderer; the remaining backend writer of chargeState is
// imageRecoveryService.settleChargeState (already pinned by E4–E6).
