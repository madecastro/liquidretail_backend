...
// Backend layout: services/ (not adgen's src/services/). Section D in adgen's
// copy pins renderer.js unsettledAtTimeout; that half is already
// scripts/verifyVideoTimeoutReconcile.js here. This file's D pins the
// fail-closed VIDEO_PROVIDER dispatch instead.
...
const { shouldResumeAttempt } = require(path.join(ROOT, 'services/atlasVideoService'));
...
const atlasSrcRaw = fs.readFileSync(path.join(ROOT, 'services/atlasVideoService.js'), 'utf8');
...
check('C1 routes/ads.js (the in-process fallback render path) passes allowResume: true explicitly', () => {
  const src = stripComments(fs.readFileSync(path.join(ROOT, 'routes/ads.js'), 'utf8'));
  assert.match(src, /veoGenerateForAd\(\{[^}]*allowResume:\s*true[^}]*\}\)/,
    'routes/ads.js must pass allowResume: true explicitly — this is the exact call site a released claim on a ' +
    'receipt-holding ad re-enters on the ADGEN_RENDERER_ENABLED-off fallback, and it must not silently rely on the default going unnoticed');
});

check('C2 [THE REGENERATE CARVE-OUT] [DECOY-RESISTANT] adRegenerateService.js passes allowResume: false explicitly', () => {
  const src = stripComments(fs.readFileSync(path.join(ROOT, 'services/adRegenerateService.js'), 'utf8'));
  ...
  // Backend extract: regenerateAd defers the work to performRegeneration
  // (shared with adgen's claimed-consumer entry). Follow that hop so this
  // check cannot be satisfied by a decoy generateForAd stuffed into
  // regenerateAd while the live path lives one function down.
  const perfCallsFromRegen = findRealMatches(regenBody, /\bperformRegeneration\s*\(/);
  ...
  const perfBody = fnBody(src, 'async function performRegeneration(');
  const runCallsFromPerf = (perfCallsFromRegen.length >= 1 && perfBody)
    ? findRealMatches(perfBody, /\brunVideoFull\s*\(/)
    : [];
  const reachesRunVideoFull = runCallsFromRegen.length >= 1 || runCallsFromPerf.length >= 1;
  const nInPerf = (perfCallsFromRegen.length >= 1 && perfBody)
    ? assertAllGenerateForAdDisallowResume(perfBody, 'performRegeneration')
    : 0;
  const nInRun = (reachesRunVideoFull && runBody)
    ? assertAllGenerateForAdDisallowResume(runBody, 'runVideoFull')
    : 0;

  assert.ok(reachesRunVideoFull || nInRegen >= 1,
    'exported regenerateAd must reach generateForAd — either by calling runVideoFull (directly or via performRegeneration) or by calling generateForAd itself');
  assert.ok(nInRun + nInRegen + nInPerf >= 1,
...
check('C3 [DECOY-RESISTANT] videoRouter.js threads allowResume through to atlasVideoService.generateForAd', () => {
  const src = stripComments(fs.readFileSync(path.join(ROOT, 'services/videoRouter.js'), 'utf8'));
...
// ── D. fail-closed VIDEO_PROVIDER (backend port of adgen's else-if + throw) ──
//
// Section D in adgen's copy of this harness pins renderer.js's
// unsettledAtTimeout handling. That half lives here as
// scripts/verifyVideoTimeoutReconcile.js (routes/ads.js catch) and is not
// re-derived. What THIS section pins is the sibling safety guard that
// shipped in the same videoRouter file: an unrecognised VIDEO_PROVIDER
// must THROW, never fall through to the deprecated billable Vertex path.
console.log('\n── D: fail-closed VIDEO_PROVIDER (videoRouter.js) ──');

const routerSrcRaw = fs.readFileSync(path.join(ROOT, 'services/videoRouter.js'), 'utf8');
const routerSrcForD = stripComments(routerSrcRaw);
const routerGenBody = fnBody(routerSrcForD, 'async function generateForAd(');

check('D1 generateForAd body extracts', () => {
  assert.ok(routerGenBody && routerGenBody.length > 100, 'videoRouter.generateForAd body not found');
});

check('D2 vertex is an explicit else-if, not a bare else fall-through', () => {
  assert.ok(findRealMatches(routerGenBody, /else if\s*\(\s*provider\s*===\s*'vertex'\s*\)/).length >= 1,
    "expected `else if (provider === 'vertex')` — a bare else is the silent-Vertex hole");
});

check('D3 unrecognised provider throws (fail-closed)', () => {
  assert.ok(findRealMatches(routerGenBody, /throw new Error\s*\(/).length >= 1,
    'unrecognised VIDEO_PROVIDER must throw — refusing to render costs nothing; generating on the wrong provider costs a master');
  assert.match(routerGenBody, /not a recognised video provider/,
    'the throw must name the refusal so an operator can see it');
});

check('D4 backend does not require a gemini video module (no such file here)', () => {
  assert.ok(!/require\(['"]\.\/geminiVideoService['"]\)/.test(routerSrcRaw),
    'do not port adgen\'s gemini branch — backend has no such module');
});

check('D5 throw message expects atlas or vertex only — not gemini', () => {
  assert.match(routerGenBody, /expected 'atlas' or 'vertex'/,
    "error text must list only the providers this repo can actually dispatch");
  assert.ok(!/expected 'atlas', 'gemini' or 'vertex'/.test(routerGenBody),
    'must not advertise gemini as a recognised backend provider');
});
