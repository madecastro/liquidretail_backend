...
const { decideInkOnLight, medianBandLuma } = require('../remotion/lib/plateHints');
const basePlateCrop = require('../services/basePlateCropService');
...
// ── C. ink-vote tie-break ───────────────────────────────────────────────────
check('C1 non-tie light majority → on-light, no globalLum', () => {
...
check('C2 non-tie dark majority → brand-default, unchanged', () => {
...
check('C3 tie + light global (median > 0.55) → on-light', () => {
  // 6 readings of 0.71 → median 0.71
  const plateHints = {
    samples: [
...
check('C4 tie + dark global (median ≤ 0.55) → brand-default', () => {
  const plateHints = {
    samples: [
...
check('C5 tie at exact 0.55 threshold → brand-default (not > 0.55)', () => {
  const plateHints = {
...
check('C6 medianBandLuma matches decideInkOnLight globalLum on tie', () => {
  const plateHints = {
    samples: [
      { atSec: 1, bands: { top: { lum: 0.2 }, middle: { lum: 0.8 }, bottom: { lum: 0.9 } } },
    ],
  };
  // sorted 0.2,0.8,0.9 → median 0.8
...
check('W3 Canonical uses decideInkOnLight for the global ink vote', () => {
  const src = fs.readFileSync(path.join(ROOT, 'remotion/compositions/Canonical.jsx'), 'utf8');
...
check('W5 detectClipBoxes returns faceSamples (source-text)', () => {
  const src = fs.readFileSync(path.join(ROOT, 'services/basePlateCropService.js'), 'utf8');
