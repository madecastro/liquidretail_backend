...
} = require('../src/services/quoteColourway');
const { toPrintableCustomerQuote } = require('../src/services/quoteProvenance');
const { prepareQuotePool, pickPrimaryProductQuote } = require('../src/services/layoutInputService');
const { displayNormalizeTitle } = require('../src/utils/titleNormalize');
const direct = require('../src/services/directImageRenderService');
const { gateLayoutInputQuotes } = require('../src/services/brandScriptExecutor');

const ROOT = path.join(__dirname, '..');
const SRC_QC = path.join(ROOT, 'src', 'services', 'quoteColourway.js');
...
const SRC_ROT = path.join(ROOT, 'src', 'services', 'quoteRotationService.js');
const SRC_LIS = path.join(ROOT, 'src', 'services', 'layoutInputService.js');
const SRC_DIR = path.join(ROOT, 'src', 'services', 'aiCreativeDirectorService.js');
...
// ── D2. Director flag-off (the shipped default) also colour-filters ──
{
  ...
  delete require.cache[require.resolve('../src/services/aiCreativeDirectorService')];
  const dir = require('../src/services/aiCreativeDirectorService');
  const product = {
    title: SOLUDOS_TITLE,
    reviews: [
...
check('E toPrintableCustomerQuote does not call usableColourwayQuote',
  !/\busableColourwayQuote\s*\(/.test(stripComments(
    require('fs').readFileSync(path.join(ROOT, 'src', 'services', 'quoteProvenance.js'), 'utf8')
  )));
