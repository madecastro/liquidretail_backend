 ...
 * ported here as #23). That fix did NOT stop the false positives:
 * pixel-measured production ads rendered AFTER it deployed (e.g.
 * meta_feed_4_5 +15px right/+38px bottom, pmax_square_1_1 +25px — both
 * comfortably inside) were STILL rejected by vision QC with findings like
 * "extends beyond the safe box boundaries (right > 1080px, bottom >
 * 1080px)" — numbers that are just the safe box's own boundary restated,
 * not an independent pixel read. The margin (13-38px, ~1-3.5% of frame) is
 * real but too narrow for a vision model doing approximate spatial
 * reasoning to perceive as "clearly inside".
 *
 ...
 * vision-judged. This repo has no imageRecoveryService.js (backend-only
 * static-recovery path), so the wiring here is scoped to the live render
 * path (finishPlate → firstOutput → runPostRenderQc → judgeRender).
 *
...
(async () => {
  ...
  // not silently drop a caller-supplied logoGeometry.
