...
const { ensureCloudinaryMirror } = require('../src/services/atlasVideoService');
...
(async () => {
  ...
  await check('Cloudinary URL WITHOUT /image/upload/ substring is NOT treated as Cloudinary', async () => {
    ...
    const fs = require('fs');
    const source = fs.readFileSync(path.join(__dirname, '..', 'src', 'services', 'atlasVideoService.js'), 'utf8');
