...
/**
 * PORTED from liquidretail_backend/scripts/verifyQuoteProvenanceStamp.js
 * (pre-2026-08-24 snapshot) into liquidretail_adgen.
 *
 * PORTING NOTE (read before touching B2): the backend original reconstructs
 * its "flag-off must equal the pre-fix baseline" comparison by running
 * `git show 3e4561e2:services/layoutInputService.js` INSIDE the backend
 * repo's own git history. liquidretail_adgen is a fresh extraction with only
 * ~10 commits of its own (verified 2026-08-24) — commit 3e4561e2 does not
 * exist here and never will, so that mechanism cannot be ported verbatim.
 * The ASSERTION itself (flag-off behaves exactly like the documented
 * pre-fix function) is unchanged; only the baseline's SOURCE changed, from
 * `git show <backend-sha>` to a verbatim string embedded below
 * (BASELINE_STAMP_QUOTE_ORIGINS_SRC), extracted by hand from that exact
 * backend commit on 2026-08-24 by an engineer, not regenerated from
 * adgen's current code — so it still cannot be satisfied by a
 * reimplementation that merely keeps the old shape. All other paths are
 * simple services/* -> src/services/* / config/* -> config/* adjustments.
 ...
 *
 * Defect (measured 2026-08-21 in the backend): stampQuoteOrigins derived
 * provenance from container.source only. json-ld (788 products / 18452
 * quotes) and api:yotpo (149 / 2482) carried quotesOrigin:'scraped' but 0 of
 * 20934 quotes had their own origin, so every first-party review stamped
 * 'unknown' and was dropped; the brand-tier last-resort printed a brand
 * quote on a t-shirt ad.
 *
 * Kill switch QUOTE_ORIGIN_FROM_CONTAINER (env, default TRUE in the fixed
 * backend). Flag-off is the pre-fix origin assignment. Flag-on adds one arm
 * after the existing gemini-search / source=store arms, so the change can
 * only convert a previous 'unknown'.
 *
 * Offline: no DB, no network, no API key.
 *   node scripts/verifyQuoteProvenanceStamp.js
 *
 * This worktree's node_modules is incomplete (no https-proxy-agent) unless
 * NODE_PATH points at a complete node_modules (e.g. the backend's). The
 * harness stubs that package so requiring layoutInputService does not crash
 * even without NODE_PATH. Flag reads are module-load-time constants:
 * loadLis() evicts require.cache and re-requires.
 */
...
} = require('../src/services/ratingPairAtomic');
const { toPrintableCustomerQuote } = require('../src/services/quoteProvenance');

const REPO = path.join(__dirname, '..');
const LIS_PATH = require.resolve('../src/services/layoutInputService');
...
function loadLis(flag) {
  ...
  return require('../src/services/layoutInputService');
}
...
// Verbatim `stampQuoteOrigins` from liquidretail_backend commit 3e4561e2
// (services/layoutInputService.js), i.e. the function BEFORE the
// 2026-08-21 quotesOrigin/classifier fix. Extracted by hand from
// `git show 3e4561e2:services/layoutInputService.js` in the backend repo on
// 2026-08-24 — see the PORTING NOTE above for why this is embedded rather
// than fetched live. Do not "helpfully" resync this to adgen's current
// stampQuoteOrigins; that would make B2 compare the function to itself.
const BASELINE_STAMP_QUOTE_ORIGINS_SRC = `
...
function loadBaselineStamp(normalizeQuote) {
  if (BASELINE_STAMP_QUOTE_ORIGINS_SRC.includes('QUOTE_ORIGIN_FROM_CONTAINER')
      || BASELINE_STAMP_QUOTE_ORIGINS_SRC.includes('classifyProductReviewsProvenance')) {
    throw new Error('embedded baseline already contains the new arm — re-extract from backend 3e4561e2');
  }
  return new Function('normalizeQuote', `${BASELINE_STAMP_QUOTE_ORIGINS_SRC}\nreturn stampQuoteOrigins;`)(normalizeQuote);
}
...
check('B2 flag-off identity against embedded pre-fix baseline (backend 3e4561e2)', () => {
  let baseline;
  try {
    baseline = loadBaselineStamp(lisOff.normalizeQuote);
  } catch (err) {
    throw new Error(`B2 could not reconstruct baseline (${err.message})`);
  }
  const mismatches = [];
  for (const [cLabel, container] of CONTAINERS) {
    for (const [qLabel, quotes] of QUOTE_LISTS) {
  ...
  assert.strictEqual(mismatches.length, 0, `flag-off drifted from the embedded pre-fix baseline: ${mismatches.slice(0, 8).join(', ')}`);
});
 ...
 * catches each (inherited from the backend original; unchanged).
 *
