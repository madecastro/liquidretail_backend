...
'use strict';
/**
 * PORTED from liquidretail_backend/scripts/verifyVideoCostReconcile.js
 * (pre-2026-08-24 snapshot) into liquidretail_adgen. Path adjustments only
 * (services/* -> src/services/*, models/* -> src/models/*); every assertion
 * below is unchanged from the backend original.
 *
 ...
 *
 *  1. In src/services/atlasVideoService.js, change parseAtlasSettledPrice to
...
const VID_PATH = path.join(ROOT, 'src/services/atlasVideoService.js');
const COST_PATH = path.join(ROOT, 'src/services/costTracker.js');
...
check('B3 status is a legal CostLog status (ok)', () => {
  const { COST_STATUSES } = require('../src/models/CostLog');
  const calls = [];
  reconcileVideoCostFromTerminal('p', { price: '0.9' }, {
    finalizeFlatCost: async (m) => { calls.push(m); },
    schedule: () => {},
