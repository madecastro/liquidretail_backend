 ...
 *
 * PORT from liquidretail_backend (fix/logo-safe-area). Require graph
 * re-anchored: scripts/ here load src/services/*, not services/* (adgen
 * has both src/config.js FILE and config/ DIRECTORY — a backend-shaped
 * require('../config/foo') from src/services/ would miss). Q1/Q2 source
 * scan is src/services/directImageRenderService.js. Paste site is
 * finishPlate → layers.push({ input: toPlace, top: place.top, left:
 * place.left }) — structurally the same compositor as backend.
 */
'use strict';
...
// ── Q. The inspector and the compositor share one box, one placement.
{
  const src = fs.readFileSync(
    path.join(__dirname, '..', 'src', 'services', 'directImageRenderService.js'),
    'utf8'
  );
