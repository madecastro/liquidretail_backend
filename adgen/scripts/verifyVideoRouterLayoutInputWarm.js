...
// Also stub geminiVideoService because videoRouter loads it eagerly and
// its real load pulls in the whole video import graph — none of which
// prepareStoryboard needs on the non-atlas branch.
const geminiStub = { generateForAd: async () => ({}) };
const geminiPath = require.resolve('../src/services/geminiVideoService');
require.cache[geminiPath] = { id: geminiPath, filename: geminiPath, loaded: true, exports: geminiStub };

const { prepareStoryboard } = require('../src/services/videoRouter');
...
async function run() {
  ...
  const fs = require('fs');
  const routerSrc = fs.readFileSync(path.join(__dirname, '..', 'src', 'services', 'videoRouter.js'), 'utf8');
  const atlasSrc = fs.readFileSync(path.join(__dirname, '..', 'src', 'services', 'atlasVideoService.js'), 'utf8');
