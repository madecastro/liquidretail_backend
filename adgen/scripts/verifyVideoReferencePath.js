...
console.log('\n8. Seed-text overlay guard STRIPPED 2026-09-03');
{
  ...
  // proven wrong three ways tonight. Stripped, not flag-gated: the
  // contradiction was live at flag-off. 9531ae9f B14 still emits the
  // guard on seedHasText=true; we no longer match that axis by design.
  ...
  check('builder no longer exports SEED_BURNED_IN_TEXT_GUARD_LINE',
    !/SEED_BURNED_IN_TEXT_GUARD_LINE/.test(veoSrc));
  check('builder no longer exports SEED_ON_PRODUCT_MARKS_LOCK_LINE',
    !/SEED_ON_PRODUCT_MARKS_LOCK_LINE/.test(veoSrc));
  check('OMNI noText still covers both halves (do not add / on-product fine to show)',
    /not already part of the supplied photographs/.test(veoSrc)
    && /physically present on the product in the source images are fine to show/.test(veoSrc));
