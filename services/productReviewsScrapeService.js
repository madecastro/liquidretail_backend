...
function parseLdBlocks(html) {
  ...
  // React Server Components stores keep their JSON-LD in the flight payload
  // instead of a script tag — see extractEmbeddedLdBlocks.
  return blocks.concat(extractEmbeddedLdBlocks(html));
}
...
/**
 * extractEmbeddedLdBlocks(html) → parsed schema.org objects[]
 *
 * Recovers JSON-LD that never appears as a <script type="application/ld+json">
 * element. Next.js App Router (React Server Components) stores serialise the
 * whole tag into the flight payload as data, so the markup arrives as an
 * ESCAPED STRING inside a JS array:
 *
 *   [\"$\",\"$L48\",null,{\"id\":\"seo-page-schema\",
 *     \"type\":\"application/ld+json\",
 *     \"children\":\"{\\\"@context\\\":\\\"https://schema.org\\\", …}\"}]
 *
 * gap.com is the live example: its Product node carries
 * aggregateRating {ratingValue: 4.48, ratingCount: 2741} and a script-tag
 * scan finds NOTHING, so tier 1 reported no reviews at all for a retailer
 * that has thousands.
 *
 * Approach: unescape progressively (depth is unknowable — Gap's is doubly
 * escaped) and, at each level, brace-match any schema.org object found. Brace
 * matching is string-aware so a `}` inside review text can't end the object
 * early. Bounded by MAX_EMBEDDED_SCAN so a pathological page can't spin.
 */
const MAX_EMBEDDED_DEPTH = 3;
const MAX_EMBEDDED_SCAN = 40;            // objects examined per depth level
const SCHEMA_SIGNATURE = '{"@context":"http';

function extractEmbeddedLdBlocks(html) {
  if (!html || typeof html !== 'string') return [];
  // Cheap bail: no escaped schema.org reference at all.
  if (!/\\+"@context\\+"|\\+"@type\\+"/.test(html)) return [];
  ...
  for (let depth = 0; depth < MAX_EMBEDDED_DEPTH; depth++) {
    ...
    let from = 0;
    let found = 0;
    while (found < MAX_EMBEDDED_SCAN) {
      const start = s.indexOf(SCHEMA_SIGNATURE, from);
      if (start === -1) break;
      found += 1;
      const end = matchBrace(s, start);
      from = start + SCHEMA_SIGNATURE.length;
      if (end === -1) continue;
      try {
        out.push(JSON.parse(s.slice(start, end + 1)));
      } catch { /* deeper escaping — a later depth pass may parse it */ }
    }
    if (out.length) break;               // stop at the shallowest level that works
  }
  return out;
}
