...
const HTML_SCHEMA_VERSION = '1.6.0';   // 1.6: video-overlay clip-path safety rule — diagonal-carve / clipped panel parents must not contain text children, since clip-path silently slices the leading characters of right-aligned headlines at the diagonal edge. Scoped to video-overlay mode only (image renders left untouched). 1.5: video-overlay slot detection broadened + no-hero_media-<img> rule. 1.4: video-overlay mode initial. 1.3: AVAILABLE IMAGE URLS allowlist. 1.2: validator host allowlist. 1.1: archetype I.
