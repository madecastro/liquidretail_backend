...
function buildPrompt({ canvas, concept, input, richContext, dims, videoMode = false, mediaRect = null, platformFormat = 'meta_feed_1_1', sourceText = [], sourceSubjects = [], sourcePrimarySubjectDesc = null, operatorPrompt = null }) {
  ...
  const system = [
    ...
    ``,
    `TESTIMONIAL LENGTH — these are direct-response ads read in under two seconds on a phone:`,
    `  - Use social_proof.primary_quote.snippet when it is present. It is a ≤50-char extract that never breaks a word. Only fall back to .text when there is no snippet.`,
    `  - Whatever you bind MUST be ≤60 characters and MUST end on a complete word and a complete thought. Never emit a quote that trails off mid-word or mid-clause, with or without an ellipsis.`,
    `  - If the only available quote is longer than 60 characters, pick the single strongest self-contained phrase inside it that is positive about THIS product and fits — do not chop the end off the full sentence.`,
