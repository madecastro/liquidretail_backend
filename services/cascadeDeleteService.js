...
async function cascadeDeleteMedia(mediaId) {
  ...
  // Pull this mediaId out of any Campaign.mediaIds[] that references
  // it. Bulk $pull is fast and doc rewrite per match is fine for the
  // working set we expect. (Replaces the old Collection $pull.)
  const campaignPullRes = await Campaign.updateMany(
    { mediaIds: mediaId },
  ...
  console.log(`🗑️  media-cascade done: media=${mediaId} artifacts=${Object.values(artifactCounts).reduce((s, n) => s + n, 0)} cloudUrls=${cloudUrls.length} campaignsPulled=${campaignPullRes.modifiedCount || 0} in ${Date.now() - t0}ms`);

  return {
    ok: true,
    mediaId,
    artifactCounts,
    campaignsPulled: campaignPullRes.modifiedCount || 0,
