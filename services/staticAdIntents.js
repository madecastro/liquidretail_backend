 ...
 */
const PRODUCT_FIDELITY = `PRODUCT FIDELITY — HIGHEST PRIORITY. Where product accuracy conflicts with any creative or styling instruction below, product accuracy wins. It does not relax the text instructions below, and it does not override the reserved-corner rule or the FORMAT block.

The reference photograph is the only source of truth for the product; where several are supplied the first is primary and the rest are further views of the same item. It is a reference, not a composition to copy. Feature the exact same physical item, as though it were carried into a new photoshoot and photographed again, never recreated from memory. Do not redesign, reinterpret, simplify, modernise, improve, repair, stylise, approximate or substitute any part of it. Do not infer the product from its category, its name, or anything you know about the brand: where the reference disagrees with your prior, the reference is right.

PRESERVE EXACTLY, as the reference shows it:
  — Form: shape, proportions, silhouette, geometry, contours, edges, thickness, volume. Fit and cut are form and may not be altered.
  — Construction: seams, stitching, panels, joints, hardware, hinges, closures, buttons, buckles, snaps, zips, clasps, laces, eyelets, straps, handles. A closure the reference hides under a flap stays hidden; a pocket the reference does not show is never added.
  — Material and surface: the same fabric, leather, knit, mesh, rubber, plastic, metal, wood, glass or packaging, at the same texture, weave, grain, gloss, matte, transparency and reflectivity. Never swap in a richer or cheaper-looking material.
  — Colour: the item's own colours exactly. No hue shift, recolour, bleach, tint, darkening, brightening or alternate colourway. New lighting may fall across those colours; it may not change them.
  — Graphics already on the item: logos, branding, icons, artwork, prints, typography, embroidery, embossing, engraving, decals, labels, tags — same wording, lettering, placement and scale, reproduced from the reference, never redrawn from imagination and never resized, restyled or swapped for a different mark. Preserving marks already on the item is never licence to place a brand mark anywhere else in the frame.
  — Details: pockets, collars, cuffs, hems, soles, heels, handles, bezels, screens, lenses, caps, applicators, chains, gemstones, grips, wheels, ports, vents, sensors and anything of that kind. Every feature visible in the reference appears unchanged; no feature absent from it is added.
  — Condition: wrinkles, folds, creases, wear, polish, finish, surface imperfections, and the shadows the item casts on itself. Do not smooth, clean or otherwise “improve” it.

NEVER substitute a similar-looking version, merge the item with another design, or produce a newer, cleaner, alternative, special-edition or differently-sized variant. If part of the item is hidden, infer only the minimum plausible geometry consistent with what is visible — geometry only, never a graphic, label or marking.

THE PERSON. If the reference shows a person, that same person appears in your advertisement, wearing or holding the item the same way. Keep their face, hair, skin tone, build and identity — do not replace them with someone else, and do not remove them to show the item on its own, on a hanger, a mannequin, a surface or a flat lay. Their pose, expression, hands and framing are yours to direct. This is a product rule, not a casting rule: when the wearer is replaced the item measurably drifts with them — closures appear, logos get restyled, colour shifts — so the person is held for the product's sake. If the reference shows no person, adding one is your choice.

SCALE AND FRAMING. Give the item approximately the same share of the frame as the reference, within about a tenth, from a similar camera distance and perspective. Do not zoom or crop dramatically tighter or wider. Compose around the item at that size: fit the environment to the product, never the product to the environment. This governs the item's size within the frame, not the frame itself — the output's dimensions and aspect are fixed by the FORMAT block below.

WHAT MAY CHANGE — everything that is not the item or the person wearing it: pose, expression, hands, environment, background, set, props, styling, lighting, shadows, mood, camera angle, focal length, depth of field, the scene's colour grading, and the typographic treatment of the copy below. Build a new scene around the item; do not reuse the reference's background or lighting. Deliberately NOT on that list: the item's size in frame, the camera's distance, whether the item is worn, and who wears it.

MAKE IT GOOD. This should read as work a premium agency shipped, not a stock photograph and not a filled-in template. Intentional lighting, art-directed typography, deliberate whitespace, the product as the primary focal point, premium and editorial. Put the inventiveness in the photography, the light and the typography — never in the product and never in the claims.

BEFORE YOU FINISH: the item is recognisably the identical physical object, its colour, branding, materials and construction match the reference, and nothing is added or removed; the person from the reference is still the one wearing it; the item holds roughly the same share of the frame; and every string given below appears exactly once, spelled exactly, with no other text anywhere. Fix any failure before finishing.`;
