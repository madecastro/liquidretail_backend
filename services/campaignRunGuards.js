...
// QC-failed-but-kept-asset ad. adPhase.js (2026-08-31) is the file that now
// owns BOTH of those distinctions in one place — see its header for the
// three-way drift (this function, the Product Ads tile, Slack's run feed)
// that motivated it — so classifyRunAdOutcome is retrofitted below to read
// deriveAdPhase()'s answer instead of re-deriving isAdHonestlyDelivered /
// isQc itself. isVideoTitlingSettled/adTitlingTruth.js is still the ultimate
// source of truth; it is just reached through adPhase.js now, not directly.
 ...
 *     it (extending adPhase.js to expose a "claim held" signal would just be
 *     re-deriving `!!ad.claimedByWorker`, which the old switch never
 *     consulted either, for no precision gain here).
 *
 * Proven equivalent against every fixture in
 * scripts/verifyTitlingDeliveryTruth.js Group C, and pinned structurally +
 * behaviourally by scripts/verifyAdPhaseParity.js.
 */
