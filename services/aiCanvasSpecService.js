...
const SPEC_SCHEMA_VERSION = '2.11.0';  // 2.11: video-source topology mandated full-bleed media + overlay-only chrome — media zone rect MUST be the full canvas with no clipPolygon, all chrome (panel/text/cta/logo/badges/quote_card/proof_bar) positioned as overlays that don't cover the full media. Eliminates pillarbox / letterbox (smart-crop now matches canvas aspect on ae79285), cover-the-video chrome (no full-canvas panels), and diagonal-carve covered-video bug (chrome carves the chrome shape, not the media). 2.10: slot tightened to 'product.hero_media' specifically. 2.9: video-source media zone REQUIRED rule. 2.8: archetype I added.
...
function buildPrompt({ input, template, aspectRatio, creativeStyle, richContext, directionConcept = null }) {
  ...
  const system = [
    ...
    `CRITICAL — VIDEO SOURCE MEDIA (full-bleed + overlay-only topology): when source_media.file_type === "video" (check the FULL CONTEXT), the spec MUST follow this exact topology — there is no negotiation:`,
    `  1. EXACTLY ONE kind:'media' zone with slot:'product.hero_media' specifically (NOT an alt-crop slot like product.hero_media.crops.*, NOT product.lifestyle_image, NOT product.product_image). Its rect MUST cover the FULL canvas — rect = {x:0, y:0, w:<canvas.width>, h:<canvas.height>}. NO clipPolygon on the media zone. The video plays edge-to-edge as a full-bleed transparent slot.`,
    `  2. ALL OTHER zones (panel, text, cta, logo, eyebrow_rules, badges, badge_row, quote_card, proof_bar, product_card, etc.) are OVERLAYS positioned WITHIN the canvas at rects that DO NOT cover the entire media. The composition variety per concept lives in HOW these overlays are arranged on top of the full-bleed video — NOT in where the media zone sits. Patterns that work:`,
    `     - Bottom panel band (rect.y ≥ 600, height 200-400) holding headline + cta + eyebrow`,
    `     - Floating quote_card or proof_bar in a corner or centered`,
    `     - Side strip panel (left or right half-width vertical) holding stacked chrome`,
    `     - Corner cards for stats, ratings, badges, creator handles`,
    `     - Top edge eyebrow rules + bottom edge cta chip`,
    `     - Diagonal-carve OVERLAY chrome (the diagonal slices the panel shape, not the media — text in sibling positioned divs, see HTML Gen rule #6)`,
    `  3. DO NOT emit a media zone with an inset rect (e.g. 30% of canvas, bottom-right corner, 50/50 split). That creates letterboxing in the rendered MP4 because the video composite output dimensions track the slot rect's aspect.`,
    `  4. DO NOT emit a clipPolygon on the media zone. That creates the covered-video bug where the chrome's diagonal carves the video out behind it (the visible playback shows nothing in the carved region).`,
    `  5. DO NOT emit a panel or chrome zone whose rect covers the full canvas (1000×1000) with a non-transparent panel_bg. That hides the video entirely. Chrome zones are sized to the text/element they hold, not to the canvas.`,
    `  6. DO NOT emit an archetype I (UGC × Product split / "ugc_x_product_split") spec for a video source — that archetype requires two media zones, and the video composite flow only fits one slot. Pick a different archetype's chrome composition instead.`,
    `Static-source media (image) is unconstrained by this rule — the original slot freedom (inset, alt-crop, lifestyle_image, etc.) applies for image sources only.`,
    ``,
