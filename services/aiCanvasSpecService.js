...
function buildPrompt({ input, template, aspectRatio, creativeStyle, richContext, directionConcept = null, platformFormat = 'meta_feed_1_1' }) {
  ...
  const system = [
    ...
    ``,
    `ZONES ARE SEMANTIC, NOT PIXEL-LEVEL LAYERS. The downstream HTML generator translates your spec into CSS where any element can have any background, any z-index, any opacity, any blend mode. Concretely that means:`,
    `  - You can declare a kind:'panel' zone whose rect COVERS the media zone (full-bleed scrim, dominant editorial spread, tinted-photo magazine layout) if your design calls for it. The HTML generator renders panels with translucent opacity / blend modes so the photo reads through. No enforced "blanking" — the LLM materializing has full CSS expressiveness.`,
    `  - Text zones can sit directly on media OR inside a panel. You don't need a panel zone behind every text region — text can be legible against a photo via shadow / outline / careful positioning, OR can have any background via the downstream CSS.`,
    `  - Use rect + visual_direction.glass_level + style_variant to communicate INTENT (heavy scrim vs floating card vs editorial backdrop vs full-bleed photo backdrop); the HTML generator picks CSS to match.`,
    `  - There's no enforced z-index hierarchy and no "panel can't cover media" rule. Compose any combination of zones that serves the archetype.`,
    ``,
    ...
    `CRITICAL — VIDEO SOURCE MEDIA: when source_media.file_type === "video" (check the FULL CONTEXT), include exactly ONE kind:'media' zone with slot:'product.hero_media' specifically (NOT an alt-crop slot like product.hero_media.crops.*, NOT product.lifestyle_image, NOT product.product_image). The renderer's composite chain targets this canonical slot to extract the source video for compositing. The zone rect can be ANY position/size on the canvas — full-bleed hero, side strip, corner inset, diagonal carve via clipPolygon, magazine editorial inset — pick what serves your chosen archetype. The render pipeline composites the source video at canvas aspect with content-aware cropping, so the video fills the canvas regardless of where you place the media zone; the media zone rect communicates your "video focus area" design intent. Two remaining constraints:`,
    `  - DO NOT emit a panel or chrome zone whose rect covers the FULL canvas (1000×1000) with a non-transparent panel_bg. That hides the video entirely. Chrome zones should be sized to the text/element they hold, not blanket the canvas.`,
    `  - DO NOT pick archetype I (UGC × Product split / "ugc_x_product_split") for video sources — that archetype requires two media zones, and the video composite flow only fits one slot. Pick a different archetype.`,
    `Static-source media (image) is unconstrained by this rule.`,
    ``,
