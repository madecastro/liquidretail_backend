...
const SPEC_SCHEMA_VERSION = '2.10.0';  // 2.10: video-source media zone slot tightened — must be slot:'product.hero_media' specifically (no alt-crops, no lifestyle_image substitute). HTML Gen's video-overlay detector matches on this single canonical slot; non-canonical slots leaked the transparent-slot instructions and the LLM baked the source frame into the chrome. 2.9: video-source media zone REQUIRED rule. 2.8: archetype I added. 2.7: enriched signal payload.
...
function buildPrompt({ input, template, aspectRatio, creativeStyle, richContext, directionConcept = null }) {
  ...
  const system = [
    ...
    `CRITICAL — VIDEO SOURCE MEDIA: when source_media.file_type === "video" (check the FULL CONTEXT), you MUST include at least one kind:'media' zone with slot:'product.hero_media' specifically (NOT an alt-crop slot like product.hero_media.crops.1_1 / .4_5 / .9_16 / etc., NOT product.lifestyle_image, NOT product.product_image — the renderer's video-composite chain targets the canonical 'product.hero_media' slot as the transparent-rect compositing site, and non-canonical media slots cause the downstream HTML generator to skip transparent-slot instructions, which makes the source frame get baked into the chrome as an <img> and cover the playing video). Give the slot a non-trivial rect — minimum 30% of canvas area — and the video plays through it. Without this exact slot/kind combination the video is INVISIBLE during playback (frozen first frame or fully-opaque overlay), which is the wrong outcome for an operator who deliberately selected a video source. Even for typographic-dominant, magazine/editorial, or stat-led archetypes that would normally skip support_media, when the source is video you MUST allocate an inset / band / corner kind:'media' + slot:'product.hero_media' zone. Adjust the typographic emphasis around it; do NOT omit it or substitute an alt-crop slot. Static-source media (image) is unconstrained by this rule.`,
    ``,
