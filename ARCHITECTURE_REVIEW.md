...
- **Authenticated-tenant remote code execution — FIXED 2026-08-03.** The brand `styleScript` preview path compiled user-supplied JavaScript with `vm.compileFunction` and **no** `parsingContext` — which does not sandbox anything. The compiled function resolved free identifiers against the real global object, so its first line could be `process.mainModule.require('child_process').execSync(...)`. Any logged-in customer (or anyone who phished one session) reached it via `POST /api/brand/:id/preview-script`. Confirmed by executing the exact call and by tracing the route. **Closed by an `engine !== 'remotion'` → 400 guard on that route**, which shuts all three reachable doors — see GEN-1 below for why the originally prescribed fix was insufficient. *(GEN-1, GEN-2 — GEN-2 is a corollary and is now unreachable by the same guard.)*
...
**GEN-1 — Brand `styleScript` is executed, not sandboxed → authenticated-tenant RCE. FIXED 2026-08-03 (route guard landed).** `services/brandScriptRunner.child.js:139` calls `vm.compileFunction` with no `parsingContext`; the parameter list controls the function's arguments, not its scope chain, so the body resolves `globalThis`/`process`/`process.mainModule.require` against the live V8 context. Reachable by any tenant via `POST /api/brand/:id/preview-script` → `brandScriptExecutor` `runChild`. **SECURITY.**

*Correction to this entry's original fix — it was insufficient.* **Three** doors reached the VM, not one, so deleting the `bodyScript` branch alone would have left a two-request exploit:

1. `body.script` → forces `engine='canvas'`
2. `body.engine:'canvas'` → the `bodyEngine` escape hatch, which short-circuits **before** `resolveTitlingEngine` is consulted
3. a `styleScript` / `styleScriptVertical` / `styleScriptLandscape` persisted earlier via `PATCH /api/brand/:id` (all three allow-listed with **no validation**), then previewed with `{engine:'canvas'}` and no `body.script` at all

*Fix as landed:* a single `if (engine !== 'remotion') return 400` immediately after the engine resolution in `routes/brand.js` (search `SECURITY (GEN-1)`). It closes all three doors, stays closed if `resolveTitlingEngine` is ever un-hardwired, and modifies no existing line. Nothing live regressed — the only UI caller, `StyleOverridesCard.tsx`, is commented out of `Brand/index.tsx` at both its import and its usage. Pinned by `scripts/verifyPreviewScriptGuard.js` (8 checks, revert-proven: removing the guard fails 3).

*Note for anyone revisiting:* adding `parsingContext` would **not** have fixed the sink — the injected params (`helpers`, `canvas`, …) are parent-realm objects, so `helpers.clamp.constructor("return process")()` escapes a fresh context anyway. The route guard is the control, not the VM options. Long-term, make brand styling a validated *data* spec (as `titleStyleSpec`/`metaCascades` already are) and never ship executable brand code in-process.
