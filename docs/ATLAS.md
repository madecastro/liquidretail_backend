...
**REMOVED 2026-08-28:** `services/openaiImageService.js` and `services/whisperService.js` used to be listed here as direct-provider exceptions (mask inpainting; Whisper per-segment timestamps feeding the also-deleted `nerService.js`). Both rows were stale — this table documented the provider-choice rationale for code that, independently re-verified, had zero callers anywhere in the repo (the whisper/NER video-detect callsite was already removed 2026-08-17; see `pipelines/detect.js`). Both files were deleted as confirmed dead code rather than kept "for standalone use elsewhere," which did not hold up under a repo-wide grep. If mask inpainting or Whisper-timestamp transcription is needed again, it will need re-implementing against the then-current Atlas model catalog, not restored from git history — check Atlas for mask-capable edit models and ASR timestamp shapes fresh rather than trusting this note's 2026-07-21 findings.
...
| A handful of billable-but-entirely-unledgered call sites found by this audit (`atlasTextService.generate` — three `routes/brand.js` title-spec call sites — plus a few raw `generateContent`/grounded-search calls that bypass `trackedGenerate`) | **NO ROW AT ALL** | **The `generateContent`/grounded-search half is CLOSED as of 2026-08-19** — `match()` in the first pass, then `categoryReviewsService` + `productDetailsService.fetchReviewSummary` in the second; all now route through `trackedGenerate`. `atlasTextService.generate`'s three `routes/brand.js` title-spec sites are STILL unledgered and still out of scope. Originally flagged, not fixed here — this is a bigger change (wiring a brand-new charge point) than "reconcile an existing estimate," and is out of scope for this pass. See the audit report / spawned follow-up. |
