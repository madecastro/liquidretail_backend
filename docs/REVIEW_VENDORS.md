...
Grounding was 92-94% of each grounded row that day (93.6% and 92.4% measured).

⚠️ **That $0.035/request surcharge default no longer exists, as of the SAME
DAY** — a separate, deliberate session change (see
`GROUNDED_SEARCH_COST_PER_REQUEST_USD` in `services/costTracker.js`): measured
grounded volume runs 13-19/day against Google's 1,500/day free allowance, so
the ledgered default is now **$0**, with a human-facing alert instead of
auto-repricing if volume ever approaches the allowance. The measurement above
is real and honest for what it measured; it is not today's price. What
`groundedRequests` and not token math still makes honest, regardless of which
number the surcharge currently holds: the free-allowance counter
(`noteGroundedRequests`, wired automatically into every `trackLlmCall`) and
any future repricing both depend on `groundedRequests` being correctly
declared on these rows — which is what this fix actually guarantees, and
what `scripts/verifyGroundedGeminiLedger.js`'s F0/F4 now check
regime-independently rather than by asserting a specific dollar ratio. The
