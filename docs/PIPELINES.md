...
| `RENDER_CONCURRENCY` | **`24`** | Parallel static ads in `runRenderLoop`. File raised 4→8 on 2026-08-02; **live in prod 2026-08-03** when the dashboard pin of 4 was deleted (see §9 / CLAUDE.md §4a). Raised 8→24 on 2026-08-04 (owner-directed: "the renders should all go out to atlas at the same time") — a wave size under the now-uncapped claim, same spend at a higher rate. **Unmeasured above 8**: watch for 429 backoff on a first full-size run |
...
- **Stop archives, and it is scoped to the STOPPING RUN (corrected 2026-08-18).** Two writes at the end of `runRenderLoop`'s `if (cancelled)` block: this run's claimed-but-never-dispatched tail (`status:'rendering'`) and this run's own minted-but-unclaimed backlog (`status:'queued'`). It used to archive `{ campaignId, status:'queued' }` — **every** queued ad on the campaign — which destroyed other runs' pending work and every mint leftover waiting for a "Generate more". Ownership is `Ad.campaignRunIds` (stamped at mint by `mintedCampaignRunIds`, `$addToSet`'d at claim); there is no persisted `generationRunId` field on `Ad`. Both filters are pure exported builders in `services/adArchiveDigest.js`, both compose `receiptFree()` + `renderUrl` empty, and both fail closed on a missing `runId`. A receipt-holding row is left in `rendering` so `bootRecoveryService` can still collect the master we already paid for. Archive never re-queues — a re-queued cancelled ad bills on the next Generate.
- **Archiving an ad RELEASES its identity digest (money — 2026-08-18).** `adSchema.index({campaignId, identityDigest},{unique:true})` is not partial, so an archived row squatted its identity slot forever; because video digests omit `generationRunId`, a never-billed archived video identity could never be re-minted. Every archive site (`runRenderLoop` Stop ×2, `queuedArchiveSweeper`, `ad.archive`, `ad.bulkArchive`, `PATCH /api/ads/:id`, `scripts/purgeQueuedAds.js`) now goes through the single helper `services/adArchiveDigest.js`, which moves `identityDigest` → `Ad.preArchiveIdentityDigest` and writes an `archived:<_id>` tombstone — **only** for rows proven receipt-free with no `renderUrl`. Paid / delivered rows are archived but keep their digest, so the index goes on protecting them from a re-bill. **So do `status:'rendering'` rows by default** — a provider charges at SUBMIT and the receipt is written after the POST returns, so a billed ad is receipt-free for one HTTP round-trip and `renderAttempts` cannot see it (`$inc`'d when a render ends, not when it starts). The only site that opts out (`allowRenderingRelease: true`) is Stop's undispatched tail, whose ids are `p.queue.slice(p.next)` — provably never dispatched. Archiving a `rendering` row also RECORDS `wasRendering`, because `ad.restore` sends a renderUrl-less archived row back to `queued` and the marker would otherwise be lost with the status. Which requeue sites stamp and which are exempt is settled in one place — the `REQUEUE_SITES` ledger in `services/adArchiveDigest.js`; every site spreads either `REQUEUE_MARK` or `PRE_DISPATCH`, and each of the four exemptions has a structural proof pinned by E15a-d. ⚠️ **But `rendering` is NOT the only place that window is reachable, and an earlier revision of this bullet wrongly said it was.** Every `rendering`→`queued` **requeue** site moves such a row out of `rendering` with no receipt wait — `worker.js`'s 15-min reaper, `processAlerts`' SIGTERM orphan persist, `/generate`'s and `/runs`' crash handlers, `claimAdsForRun`'s CLAIM ANOMALY release, and `/generate`'s CAS-lost release (which deliberately NULLs `renderStage`). Three further clauses on the gate: the durable `Ad.wasRendering` marker, stamped by each requeue site's own **awaited** write and never cleared (`renderStage` is best-effort — `services/adStage.js` is fire-and-forget *by contract* — and `renderAttempts` is `$inc`'d only when a render *ends*, so neither survives the crash this guards; a never-claimed mint leftover carries none of the three and still releases); `imageGeneration` must be null/absent or an **object** (`$type`), because on a string/array parent `$imageGeneration.predictionId` is *missing* and a real static receipt would read as "no receipt"; and **a tombstone may never sit on a non-archived row** — the status flip rides the same `$cond` as the digest restore, so a tombstoned row with nothing to restore stays archived instead of reaching `queued` with a placeholder identity. Restore paths (`PATCH` → draft/live, `ad.restore`, `purgeQueuedAds --restore`) hand the digest back and answer a duplicate-key collision with a refusal (409), never by leaving the tombstone live on a claimable row. Pinned by `scripts/verifyArchiveDigestRelease.js`, which derives the archive- and requeue-site lists by scanning rather than hardcoding them. **Residuals:** rows requeued before this shipped carry no marker and fall back to `renderStage`; a derive that entered its wait loop is marked, so that free 1:1 identity stays squatted (never billed, so releasing would be safe — keeping it is the fail-closed direction); and there is **no backfill**, so rows archived before this change still squat their digests (a second `PATCH → archived` heals one).
...
> **Default path (Meta):** product campaigns queue **one deterministic video ad per product** (hero seed or operator-ordered catalog stack) — `resolvePreset('meta_video'|'meta_all')` returns `videoFormats: [META_VIDEO_MASTER]` only. **Google PMax is different:** `google_video` / `google_all` queue **two billable Omni masters** (9:16 + 16:9) plus one **free derive-only** 1:1 crop of the 9:16 master — see [*Google Performance Max video*](#google-performance-max-video-phase-a-2026-08-10) below. **A mixed Meta+PMax run shares ONE portrait plate across both platforms** (still two billable, but the 9:16 is Meta's), same section. The Creative Director no longer drives video by default — it serves **static image ads** and **opt-in video variants** only (backend PRs #11/#12/#13; wizard controls frontend PR #10).
>
...
- **Reference stack (what the model actually receives on the dormant fallback path)** — `buildReferenceImages` (`atlasVideoService.js:1917`) sends the first **3 DISTINCT** views: primary seed at position 0, then catalog mirrors in the order the caller supplied (`buildReferenceImages` does not sort — it trusts the arrival order of `catalogMedias`). In production, adgen's own renderer builds the reference stack instead. Count comes from `DEFAULT_REFERENCE_IMAGE_COUNT = 3` (`:800`), overridable per brand/product via `videoSettings.referenceImageCount` up to `MAX_REFERENCE_IMAGE_COUNT = 7`, and always clamped to the resolved model's `maxReferenceImages`. An explicit operator pick list defines its own count (picking 5 means 5, not "5 truncated to 3") but is still clamped to the stack budget, with a warn.
...
>
> **MIXED Meta+PMax runs share the portrait plate UNCONDITIONALLY as of `fba81588` / 2026-09-03 (owner: one 9:16 minting regardless of hook-first).** `resolvePreset` still returns all three surfaces — that number is a SURFACE count, not a bill — but `planDeterministicVideoAds` marks `pmax_video_9_16` a **free derive of `meta_stories_9_16`**, so a mixed run bills **2 masters ($1.80), not 3 ($2.70)**, at an unchanged 21 Ads. `VIDEO_HOOK_FIRST_PROMPT` / `PMAX_VIDEO_DIRECTIVES` still pick camera-prompt *text*; they no longer gate whether sharing happens. ⚠️ **SUPERSEDED:** from 2026-08-18 until `fba81588` sharing also required the hook-first switch ON, and the 2026-08-20 FILE default of `false` therefore billed 3 / $2.70 on every mixed run. Do **not** quote $2.70 as today's mixed cost unless `UNIFIED_VIDEO_9_16_MASTER=false` or the Meta 10s floor is off. The two formats declare byte-identical `deliveryDims` (1080×1920) and differ only in their Remotion safe ZONE, resolved per row at titling time — **one plate, two titlings**. The decision is `resolvePortraitMasterFormat` (`campaignAdsGenerationService.js:717-746`), taken once at mint and stamped as `deriveFromMaster`; the renderer only ever reads the stamp. It **fails closed on four conjuncts** (kill switch `UNIFIED_VIDEO_9_16_MASTER`, Meta master in this run, PMax portrait in this run, Meta 10s floor active). Camera coherence is **not** a conjunct — `isSharedPortraitPlatePromptCoherent` is still exported but is not called from the decision (pinned F2). A **PMax-only run still pays for its own 9:16** — a derive with no master ships nothing, which is worse than $0.90. ⚠️ Do **not** drop `pmax_video_9_16` from `GOOGLE_VIDEO_MASTERS` to express this: `isGoogleVideoMasterRun` keys on its presence, so removing it also stops `pmax_video_1_1` minting. ⚠️ The shared plate is 10s because **Meta video is now floored at 10s universally** (owner directive 2026-08-18 — the wizard posts `8` and used to win, so the documented "Meta is 10s" was false on every UI run). Google rejects PMax video under 10s, so that one rule is also what makes the shared plate a legal PMax asset — there is deliberately no mixed-run duration branch. The two are coupled: `META_VIDEO_DURATION_SEC=0` reverts the Meta floor and `resolvePortraitMasterFormat` then **refuses to share** rather than ship an 8s PMax asset. Digest-safe — duration is hashed for Google formats only, so no stored Meta digest moves; the flip side is that a campaign with an existing 8s Meta video ad **keeps it** (the 10s row hashes identically and is swallowed). Pinned by `scripts/verifySharedPortraitMaster.js`.
...
**WAS Phase A known gap; CLOSED Phase B (dormant fallback path).** Zones are resolved per
`platformFormat` and threaded to the composition there:
`pmax_video_9_16` → `verticalYt`, `pmax_video_16_9` → `landscapeYt`,
`pmax_video_1_1` → `squareYt` (`brandScriptExecutor` / `remotionRenderService`
/ `remotion/*`). Meta surfaces still use Meta zones. In production, adgen's
ported titling code does this resolution instead.
...
generic 8s `canonical-<stage>`). On the dormant fallback path, `renderWithRemotionAndSave` resolves the
...
the composition. (In production adgen's ported titling logic does this instead.) The unstaged master (stage null) still uses today's
...
- **Shared cache-first resolver** (`services/videoReferenceResolver.js`, `d8ad7285`+): `resolveVideoReferenceForMedia({ media, aspectRatio, brand })` is the helper **every video-model ref builder should call** so a non-Atlas path cannot silently skip the DINO cache. Tiers: reframe-cache → on-demand YOLO crop → source-native `c_fill` (`326fb270`). Backend Atlas Omni still uses `reframeReferenceForAspect` (write path); this helper is the read-side contract for adgen direct-Gemini / future models. Pinned by `scripts/verifyVideoReferenceResolver.js`.

**Reframe ladder** (`reframeReferenceForAspect`, ported from ReachSocialLLMExpander `runSafeZoneReframe`; replaces a prior unmasked-whole-image edit that had no subject-preservation clause and produced visible artifacts — warped logos, mangled label text, drifted product shape). Persisted entries carry `ladderVersion: 'reframe-v2'` (`REFRAME_LADDER_VERSION` at `atlasVideoService.js:195`). ⚠️ **`reframe-v3` was reverted the same day it shipped (`6fcd5e55`)** — live adgen still tags `reframe-v2` and treats `!==` as stale, so backend-written v3 entries were re-derived via composite-outpaint and overwrote $0 force-crops. Source-native pad + force-crop **code stayed**; only the tag reverted. Do not re-bump until adgen matches. Historical first-port tag was `uncrop-v1`.
...
6. **Validate output** — byte floor (512B) + aspect ratio within `REFRAME_RATIO_TOLERANCE` of target (ratio only — pixel size is never compared). Rejected → falls through to the pad decision.
7. **Pad-or-crop fallback** — `$0` via `seedPadDecision` (`detectBorderFill`, same check as step 3). Pad **only** when the sampled border is genuinely flat/uniform and yields a solid hex fill that is invisible against the backdrop (`pad-solid` → `method: 'pad-fallback'`). Otherwise the ladder **crops** — the blurred-cover pad was deleted 2026-08-12 (PR #155: "never hand the model a letterboxed seed"). Reason: the seed is reference input to an image-to-video model, so any visible band in the seed is baked into the rendered video where no downstream crop can remove it.
...
- Atlas image-to-video (**default Gemini Omni**; Grok / Veo slugs in `MODEL_CAPS` as fallbacks) — Omni RPS **unpublished/unmeasured**. On the dormant fallback path, same-model video submits are paced by `pacedModelSubmit` (`ATLAS_SUBMIT_SPACING_MS` default 1200ms); in production adgen's renderer submits instead and paces independently. Grok (aspect-fallback only) stays ≤1 RPS via `GROK_MAX_RPS` floor regardless of `VEO_CONCURRENCY`.
- **Was false:** "429s if concurrency > 1 / keep VEO_CONCURRENCY=1". That justification belonged to retired direct Google Veo and to Grok’s documented 1 RPS — not the primary Omni path. Current default **`VEO_CONCURRENCY=24`** — the **submit+poll half only** since 2026-08-05 (1→4 on 2026-08-02 as a probe, 4→12 on 2026-08-05 once titling moved behind its own permit, 12→24 on 2026-08-20). Widening this half is low-risk by the raises’ own reasoning: an Omni poll is ~2min of idle waiting (p50 117s / p99 247s) and no Omni 429 has ever been recorded. What remains unmeasured is Omni’s RPS, which is unpublished. The memory-bound half is `REMOTION_QUEUE_CONCURRENCY=8` — that is the one to validate against the web-service memory graph before raising.
- Resolution default **`ATLAS_VIDEO_RESOLUTION=1080p`** — same list price as 720p on Omni; matches Meta `deliveryDims` (all 1080-wide).
- **Submit count (money):** Meta live presets = **1** Omni master per product. Google `google_video` = **2** Omni masters per product (9:16 + 16:9 at 10s); the 1:1 is free crop+retitle. **A MIXED Meta+PMax run = 2 total, not 3** (unconditional as of `fba81588`; 3 only if `UNIFIED_VIDEO_9_16_MASTER=false` or the Meta 10s floor is off) — the portrait plate is shared, so the marginal cost of adding PMax beside Meta is one 16:9 master (**$0.90**), and a mixed video run costs **$1.80** where it used to cost **$2.70**. **MEASURED unit costs (Phase B, settled prediction prices):** 10s Omni master on the **developer** model = **$0.90** (not the $1.20 `MODEL_CAPS` formula — that overstates developer by ~33%); 3-size PMax static fan-out ≈ **$0.199**/concept. Full kit (3 concepts × 3 statics + 2 masters) ≈ **$2.40** standalone / ≈ **$1.50** marginal beside a Meta run that already paid for 9:16. Earlier planning figure ≈$2.6 is **superseded**. No full end-to-end app run yet — see [*Measured PMax unit costs*](#measured-pmax-unit-costs-phase-b-live-submits-2026-08-1011) and `session.md`.
...
| `VEO_CONCURRENCY` | **`24`** | Self-imposed in-flight video ads per run — **submit+poll only** since the 2026-08-05 split (1→4 2026-08-02, 4→12 2026-08-05, 12→24 2026-08-20). No Omni 429 ever recorded on this half. The real titling/memory cap is `REMOTION_QUEUE_CONCURRENCY` — see [§8](#8-concurrency-knobs) |
...
Single resolver: `services/concurrency.js` (frozen `concurrency` object; boot logs the table). `defaults.env` + code defaults agree on the numbers below as of 2026-08-20. They are asserted against each other by `scripts/verifyConcurrencyConfig.js` — a change here that script does not pin is a docs bug. (`scripts/verifyTitlingPermit.js`, formerly the second harness pinning this table, is deleted along with `VEO_TITLING_CONCURRENCY` and `services/semaphore.js` — see the removed-row note below.)
...
| `REMOTION_QUEUE_CONCURRENCY` | **8** | SELF — **the real titling limit and the number to move carefully.** Still live — this is `services/remotionRenderService.js`'s own render queue, used by `routes/brand.js`'s manual retitle endpoints, unaffected by the removal above. Simultaneous Remotion renders: a headless Chrome page + an ffmpeg 1080p encode, IN the web process. Failure mode is **not** a provider 429 — it is RSS exhaustion → Render autoscale (60% CPU+mem) → process replacement → a PAID Omni master stranded mid-titling (~$1.00 each). Was 4, the only concurrency this process had actually survived (the old combined `VEO_CONCURRENCY`), and never an RSS measurement; raised 4→8 (2026-08-20) as one deliberate doubling — 16 was floated and rejected for that reason. **Validate 8 against the web-service memory graph across a full run before raising further** (SPEC max 16; one measured render = 76.2s, and 1080p is 2.25x the pixels of 720p) |
...
| Concurrency | `WORKER_CONCURRENCY`, `RENDER_CONCURRENCY=24` (4→8 went **live 2026-08-03** when the dashboard pin was deleted — see above; 8→24 on 2026-08-04), `VEO_CONCURRENCY=24` (submit+poll only; 1→4→12→24), `VEO_TITLING_CONCURRENCY=48`, `REMOTION_QUEUE_CONCURRENCY=8` (the real titling/memory cap), `ATLAS_SUBMIT_SPACING_MS`, `GROK_MAX_RPS`, `MAX_CREATIVES_PER_RUN` — resolved via `services/concurrency.js` |
