...
**When the in-render wait itself expires (master still `queued`/`rendering`, 2026-08-20):** `handleDeriveMasterBackup` (extracted from `renderDeriveOnlyVideoAd`) — **never abandons the ad.** It requeues to `queued` (same zero-submit write as before) and immediately reclaims through the SAME atomic claim path stranded ads use (`requeueStrandedAds` → `claimAdsForRun`), instead of leaving the ad for an operator click or a crash-triggered sweep to notice — that passive hand-off was the actual bug (`services/strandedRunSweeper` only scans ads whose run is `failed`, and the run a derive-wait ad backs out of normally finishes `done`). It also fires one Slack notice per backup **episode** (keyed on the master, via `alertService`'s own dedupe — not one per ad, not one per poll), distinguishing a merely-queued master from one that looks genuinely stuck (`rendering` with no heartbeat past `reapStaleMin()`). The old hard cap (`MAX_DERIVE_WAIT_ATTEMPTS`, 30 cycles) no longer fails the ad — it only escalates the alert from `warn` to `error`. See `scripts/verifyDeriveWaitBackup.js`.
...
⚠️ **Related latent defect, flagged not fixed.** `enforceByteCap` uses `caps?.promptByteCap || DEFAULT_BYTE_CAP`, so any call site reaching it with `caps` absent or a falsy `promptByteCap` silently gets **4,096**. And when a prompt is still over cap after every `DROP_PRIORITY` line is gone, it logs *"Atlas will reject"* **and returns the over-cap prompt anyway — it does not truncate**, so the over-cap body is what gets submitted (contrast `enforceRawByteCap`, which does hard-truncate). A prompt measured at 4,168 bytes sits at 21% of budget on gemini-omni and is **over cap** on all four 4096-capped shapes. Open question, not chased: whether any live brand/product actually routes to a 4096-capped shape at this prompt length. The response now carries an `approximation` block naming every assumed and omitted input; the SPA renders it beside the editor.
...
Single resolver: `services/concurrency.js` (frozen `concurrency` object; boot logs the table). `defaults.env` + code defaults agree on the numbers below as of 2026-08-20. They are asserted against each other by `scripts/verifyConcurrencyConfig.js` and `scripts/verifyTitlingPermit.js` — a change here that those scripts do not pin is a docs bug.
...
| `VEO_TITLING_CONCURRENCY` | **48** | SELF — how many ads may be INSIDE the titling call at once (4→48, owner-directed 2026-08-13). **Not the memory guard**, despite the old note: it bounds the cheap prep half (`buildMetaForAd`, copy cascade, product/category reads, font resolution — Mongo and disk). `remotionRenderService` used to run a concurrency-1 promise chain regardless of this number, which is the whole 926s / 83%-idle titling tail measured on a 20-ad run. One caveat: ~$0.02 of billable face-detection vision runs inside this permit per ad, so a wide value bursts those concurrently — same total spend, higher instantaneous rate |
| `REMOTION_QUEUE_CONCURRENCY` | **8** | SELF — **the real titling limit and the number to move carefully.** Simultaneous Remotion renders: a headless Chrome page + an ffmpeg 1080p encode, IN the web process. Failure mode is **not** a provider 429 — it is RSS exhaustion → Render autoscale (60% CPU+mem) → process replacement → a PAID Omni master stranded mid-titling (~$1.00 each). Was 4, the only concurrency this process had actually survived (the old combined `VEO_CONCURRENCY`), and never an RSS measurement; raised 4→8 (2026-08-20) as one deliberate doubling — 16 was floated and rejected for that reason. **Validate 8 against the web-service memory graph across a full run before raising further** (SPEC max 16; one measured render = 76.2s, and 1080p is 2.25x the pixels of 720p) |
