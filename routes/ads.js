...
// POST /api/ads/runs/:runId/cancel — SCAFFOLDING (owner scope addition
// 2026-08-26, targeted for the NEXT merge cycle, landed early because
// `cancelling`/`cancelled` had to exist as CampaignRun.status values and a
// route before the phase model and the adgen claim-side changes that
// consume them could be finished). Mirrors routes/progress.js's
// `POST /:runId/cancel` contract exactly (cooperative, idempotent on a
// terminal run, tenant-scoped) rather than inventing a second pattern.
//
// WHAT THIS DOES today: flips the run to 'cancelling', archives its own
// remaining QUEUED backlog via the SAME pure, receipt-free-guarded helper
// Stop already uses (buildStopBacklogArchiveFilter — no new money logic,
// no code path here can touch a claimed/rendering ad), and records who
// asked. It does NOT yet reach into adgen: renderer.js/titler.js's
// claimOne() do not check this flag (that is the next-cycle, money-critical
// half — see CLAUDE.md/session notes for why it isn't done in this pass:
// it requires reading adgen commit 3ec6a41's dual-claim-race fix and the
// money-invariant harnesses named there before touching either claim
// filter, which this session did not have adgen git access to do safely).
// A run stuck 'cancelling' with claimed ads still in flight is therefore
// EXPECTED right now — the worker.js stranded-claim scan added alongside
// this (see services/adPhase.js) will still page if those claimed ads stop
// moving, same as any other stall, but nothing here forces them to stop.
//
// Idempotent: a terminal run ('done'/'failed'/'cancelled') returns the
// current doc unchanged, same as progress.js. A run already 'cancelling'
// also short-circuits (no double-archive, no duplicate errors[] entry).
router.post('/runs/:runId/cancel', express.json(), async (req, res) => {
  try {
    const brandId = req.query.brandId || req.headers['x-brand-id'];
    if (!brandId) return res.status(400).json({ error: 'brandId required' });
    try {
    ...
    const run = await CampaignRun.findOne({ runId: req.params.runId, brandId });
    if (!run) return res.status(404).json({ error: 'run not found' });

    if (['done', 'failed', 'cancelled'].includes(run.status)) {
      return res.json({ run: run.toObject() });
    }
    ...
    // Backlog archive FIRST, status flip SECOND — a claim that lands between
    // these two writes still sees `status:'preparing'|'running'` and either
    // succeeds honestly (fine, this scaffolding does not yet prevent it) or
    // the archive's own receiptFree()-composed filter already excludes it.
    // Reusing buildStopBacklogArchiveFilter, not a new query: same run-
    // scoped (never campaign-wide — the 2026-08-18 Stop bug this repo
    // already fixed once), same receipt-free guard, same digest release.
    const backlog = await archiveAdsReleasingDigest(
      Ad,
      buildStopBacklogArchiveFilter({ runId: run.runId })
    ).catch(() => null);
    ...
    run.cancelledAt     = now;
    run.cancelledBy     = requestedBy;
    run.updatedAt       = now;
    run.errors.push({
      index: -1,
      stage: 'cancel',
      message: `Cancel requested by operator — ${archivedBacklog} queued backlog ad(s) archived. ` +
        `Claimed/in-flight ads are NOT yet force-stopped by this endpoint (next-cycle work); ` +
        `this run stays 'cancelling' until they settle on their own or the reconciler catches a stall.`
    });
    await run.save();

    console.log(`🛑 [campaignRun ${run.runId}] cancel requested — archived ${archivedBacklog} queued backlog ad(s), status→cancelling`);
    res.json({ run: run.toObject() });
  } catch (err) {
    console.error('run cancel failed:', err);
    res.status(err.status || 500).json({ error: err.message || 'run cancel failed' });
  }
});
