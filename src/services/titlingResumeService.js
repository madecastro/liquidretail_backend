    ...
    try {
      ...
      if (!brand) {
        ...
        if (tooOld) {
          ...
          // GUARDED — same reason as the titled arm above: qcAndStampVideoAd
          // runs on the line above and can stamp status:'failed' on a real
          // vision-QC failure. A bare filter here overwrote it with 'draft'.
          // PARITY FIX: this arm was guarded in the backend copy of this file
          // but missed here — caught by adversarial review, not by the
          // per-repo enumeration that found the other five sites.
