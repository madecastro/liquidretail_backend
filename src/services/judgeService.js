...
// ─────────────────────────────────────────────────────────────────────────────
async function judgeDetections({ imageUrl, products, subjects, text, crops, safeRect, campaignRunId = null, brandId = null, productId = null, adId = null }) {
  const payload = {
  ...
  const response = await chatCompletion({ stage: 'judge_detections', service: 'judgeService', visionImages: 1, campaignRunId, brandId, productId, adId }, {
    model: 'gpt-4.1',
...
async function judgeExtendedCrops(arg) {
  ...
  const response = await callOpenAIWithCloudinaryRetry({
    ...
    max_tokens: 2500,
    temperature: 0.2
  }, { campaignRunId, brandId, productId, adId });
...
async function callOpenAIWithCloudinaryRetry(payload, meta = {}) {
  const delays = [1500, 4000];   // ms before each retry
  let lastErr;
  for (let attempt = 0; attempt <= delays.length; attempt++) {
    try {
      return await chatCompletion({ stage: 'judge_extended_crops', service: 'judgeService', visionImages: 1, ...meta }, payload);
    } catch (err) {
      lastErr = err;
