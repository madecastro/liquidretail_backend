...
const SPEC = Object.freeze({
  ...
  REMOTION_QUEUE_CONCURRENCY: {
    ...
    // FALLBACK LOWERED 4 -> 2 on 2026-08-25. This default only applies when
    // config/defaults.env fails to load, and 4 is the EXACT configuration that
    // OOM-killed an 8 GiB box while it held a paid Omni master (see the `why`
    // below). A fallback that fires in a degraded boot must not be the known
    // fatal value — it should be the most conservative value the process has
    // survived. The tuned operating value lives in defaults.env (currently 3).
    default: 2,
    min: 1,
    max: 16,
    ceiling: 'SELF-IMPOSED',
    why: 'Simultaneous Remotion renders — headless Chrome page + ffmpeg 1080p encode, IN the web process. This is the real titling limit and inherits the caution the old VEO_TITLING_CONCURRENCY note carried: the failure mode is not a provider 429, it is RSS exhaustion -> Render autoscale (60% CPU+mem) -> process replacement -> a paid Omni master stranded mid-titling (~$1.00 each). Default 4 was a 4x throughput win over the serial chain it replaces (926s tail -> ~230s projected) and was NOT arbitrary: 4 was the value the combined VEO_CONCURRENCY carried when it ran submit+poll AND Remotion in one lane, so it was the only concurrency this process had actually survived. RAISED 4→8 on 2026-08-20 (owner-approved), one doubling rather than the 16 originally floated, specifically because 4 was never an RSS measurement and the failure mode strands an already-paid master. Moved together with VEO_CONCURRENCY 12→24 — leaving this at 4 would have made titling a strictly harder bottleneck. 8 was to be validated against the web service memory graph on a full run before any further raise; 1080p is 2.25x the pixels of 720p and one measured render is 76.2s. REVERTED 8→4 on 2026-08-21: that validation ran and 8 failed it. A real 12-ad Pelagic video run drove the web service from 0.33 GiB idle to 7.57 GiB in two minutes and was OOM-killed at the 8Gi limit (Render server_failed/oomKilled, 10:35:54Z), stranding the paid Omni master mid-titling exactly as this note predicted. Implied ~0.9 GiB per concurrent slot, so 8 slots want ~7.2 GiB of an 8 GiB box with no headroom. Recovery worked (titlingResumeService re-titled for free; the reaper correctly spared the run) so only ~20 minutes were lost and no money was re-spent — but that is the safety net, not a licence to keep a concurrency the process cannot survive. Raising instance RAM is the prerequisite for re-raising this, not an alternative to it. CORRECTION 2026-08-25: the "~0.9 GiB per concurrent slot" implied above is WRONG by ~2.2x — it divided the 7.57 GiB peak by 8 permits when only 4 were demonstrably rendering at that instant. The real figure is (7.97 - 0.09) / 4 = ~1.97 GiB/slot. That corrected number, plus the verified fact that autoscaling is DISABLED on adgen-renderer (so the old "stay under the 60% trigger" constraint does not exist), is why the operating value moved 2 -> 3 (~5.9 GiB, 74%, ~2 GiB headroom) rather than staying at 2. 4 remains fatal on an 8 GiB box. NOTE the 1.97 was measured on the RENDERER, which also runs Atlas HTTP and static submits in-process; the dedicated titler has no competing work and may sustain more, but that must be measured there. See config/defaults.env for the full memory graph.'
  },
