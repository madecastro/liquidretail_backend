...
//      product's logo and nothing objects. This is the mechanism behind the
//      2026-08-21 `layout_safe_box` QC failures ("the caption overlay is
//      placed directly on top of the primary back logo, obscuring the brand
//      name").
...
//      So correcting the strips makes the ink vote MORE correct, not riskier.
//
...
// PANEL_CENTER_GUTTER_FRAC below: plateIntel is CJS and safeZones is the ESM
// remotion island, with no shared module graph. The harness pins every value
// equal to SAFE_ZONES, so a drift fails loudly instead of silently mis-sampling.
...
// shots this catalogue is full of.
//
...
// bandFaceOverlapFrac: that function is exported and any harness calling it
// with explicit rects would have every result inflated by changing the
// denominator inside it. Rescaling the pure function's output is
// algebraically identical (inter/(width*REF) === raw * stripH/REF) and leaves
// that call surface untouched.
 ...
 */
async function analyzePlate(platePath, { durationSec = 8, isImage = false, panelSide = null, xRange = null, brandId = null, productId = null, adId = null, campaignRunId = null, safeZoneKey = null } = {}) {
